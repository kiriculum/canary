...
def get_price_changes_per_company(companies_qs: CompanyQuerySet):
...
def get_sector_outstanding(sectors_perf: dict, companies_perf: dict) -> None:
    ...
    for sector, sector_perf in sectors_perf.items():
        for field in CompanyQuerySet.changes:
            ...
            sector_perf[f'bot_{field}'] = list(map(lambda x: (x['code'], round((x[field] - 1) * 100, 2)), bot))
            sector_perf[f'top_{field}'] = list(map(lambda x: (x['code'], round((x[field] - 1) * 100, 2)), top))


def get_market_shares():
    company_qs = Company.objects.last_top500()
    companies_perf = get_price_changes_per_company(company_qs)
    ...
    for sector, perf in sectors_perf.items():
        for change_field, sector_field in zip(sector_changes, sector_fields):
            if not perf[sector_field]:
                continue
            perf[change_field] = round((perf['sector_value_today'] / perf[sector_field] - 1) * 100, 2)
